...
export function ProjectBody({project,children}){
 ...
 return <div className="project-images" ref={root}><img src={project.image} alt={project.title+'の課題ビジュアル'}/><section className={'project-story story-'+project.id} style={{background:project.color}}><span>{project.category}</span><h2>Make a<br/>proposal<br/>matter.</h2><p>{project.detail}</p></section>{project.detailImage?<img src={project.detailImage} alt={project.title+'の課題イメージ'}/>:<div className={'project-triptych triptych-'+project.id}>{[0,1,2].map(i=><div key={i}><img src={project.image} alt={project.title+'の構成ディテール '+(i+1)} style={{objectPosition:`${i*50}% center`}}/></div>)}</div>}{children}</div>;
}
