...
const vertex=`varying vec2 vUv;uniform float speed;void main(){vUv=uv;vec3 p=(modelMatrix*vec4(position,1.)).xyz;float phase=(p.x+1.4)*.62;float wave=cos(phase);float s=clamp(speed,-2.,2.);p.x+=.25*sin(phase);p.y=p.y*(.995+.185*wave+.045*abs(s)*wave)+.08*(1.-wave)+s*.035*sin(phase);p.z+=.12+.18*wave;gl_Position=projectionMatrix*viewMatrix*vec4(p,1.);}`;
...
export function Gallery({onOpen,paused}){
 ...
 useEffect(()=>{
  ...
  [...projects,...projects].forEach((p,j)=>{const i=j%projects.length,copy=Math.floor(j/projects.length);
   ...
   const button=document.createElement('button');button.className='canvas-project';button.setAttribute('aria-label',p.title+' — 作品を見る');button.addEventListener('click',e=>{if((e.detail===0||distance<7)&&!status.current.paused)onOpen(i,button.getBoundingClientRect());});button.addEventListener('pointerenter',()=>{hover=i;});button.addEventListener('pointerleave',()=>{hover=-1;});button.addEventListener('focus',()=>{if(!down){target=current+(((i*gap-current+total/2)%total+total)%total-total/2);lastAction=performance.now();}});el.append(button);cards.push({mesh,button,texture,ctx,img,detail,p,i,copy,lastPaint:-100,lastHover:false});
  });
  ...
  const pos=new THREE.Vector3();const tick=time=>{if(stopped)return;frame=requestAnimationFrame(tick);const dt=Math.min((time-previous)/1000||.016,.05);previous=time;if(document.hidden)return;const speed=target-current;current+=speed*(reduced?1:1-Math.exp(-8*dt));
   for(const c of cards){let x=((c.i*gap-current+total/2)%total+total)%total-total/2;if(c.copy)x+=x<0?total:-total;c.mesh.visible=Math.abs(x)<camera.aspect*3.8+width;c.button.hidden=!c.mesh.visible;if(!c.mesh.visible){c.button.style.display="none";continue;}c.mesh.position.x=x;c.mesh.rotation.y=0;c.mesh.material.uniforms.speed.value=reduced?0:Math.max(-2,Math.min(2,speed));if(time-c.lastPaint>40||c.lastHover!==(hover===c.i)){paint(c.ctx,c.img,c.detail,c.p,reduced?0:time/1000+c.i*3.7,hover===c.i);c.texture.needsUpdate=true;c.lastPaint=time;c.lastHover=hover===c.i;}const outline=[];for(let k=0;k<=16;k++){pos.set(...surface(x-width/2+width*k/16,height/2,0,reduced?0:speed)).project(camera);outline.push([(pos.x*.5+.5)*el.clientWidth,(-pos.y*.5+.5)*el.clientHeight]);}for(let k=16;k>=0;k--){pos.set(...surface(x-width/2+width*k/16,-height/2,0,reduced?0:speed)).project(camera);outline.push([(pos.x*.5+.5)*el.clientWidth,(-pos.y*.5+.5)*el.clientHeight]);}const left=Math.min(...outline.map(p=>p[0])),top=Math.min(...outline.map(p=>p[1])),pw=Math.max(...outline.map(p=>p[0]))-left,ph=Math.max(...outline.map(p=>p[1]))-top;c.button.style.cssText=`width:${pw}px;height:${ph}px;transform:translate3d(${left}px,${top}px,0);clip-path:polygon(${outline.map(p=>`${(p[0]-left)/pw*100}% ${(p[1]-top)/ph*100}%`).join(",")})`;c.button.tabIndex=Math.abs(x)<gap*.6?0:-1;}renderer.render(scene,camera);};frame=requestAnimationFrame(tick);
